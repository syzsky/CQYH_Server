...
namespace 游戏服务器.地图类
{
	public sealed class 对象仇恨
	{
		...
		public 对象仇恨()
		{
			this.仇恨列表 = new Dictionary<地图对象, 仇恨详情>();
		}
		...
		{
			if (!(对象 is 玩家实例 { 隐身模式: not false }) && !对象.对象死亡)
			{
				if (this.仇恨列表.TryGetValue(对象, out var value))
				{
					value.仇恨时间 = ((value.仇恨时间 < 时间) ? 时间 : value.仇恨时间);
					value.仇恨数值 += 仇恨数值;
				}
				else
				{
					this.仇恨列表[对象] = new 仇恨详情(时间, 仇恨数值);
				}
			}
		}
		...
		public bool 切换仇恨(地图对象 主人)
		{
			...
			List<地图对象> list;
			list = new List<地图对象>();
			foreach (KeyValuePair<地图对象, 仇恨详情> item in this.仇恨列表)
			{
		...
		public bool 最近仇恨(地图对象 主人)
		{
			int num;
			num = int.MaxValue;
			List<KeyValuePair<地图对象, 仇恨详情>> list;
			list = new List<KeyValuePair<地图对象, 仇恨详情>>();
			foreach (KeyValuePair<地图对象, 仇恨详情> item in this.仇恨列表)
			{
				int num2;
				num2 = 主人.网格距离(item.Key);
				if (num2 < num)
				{
					num = num2;
