		...
		public override void 执行命令()
		{
			if (游戏数据网关.角色数据表.检索表.TryGetValue(this.角色名字, out var value) && value is 角色数据 角色数据)
			{
				if (!(角色数据.删除日期.V == default(DateTime)) && 角色数据.所属账号.V.删除列表.Contains(角色数据))
				{
					...
					{
						主程.添加命令日志("<= @" + base.GetType().Name + " 命令执行失败, 角色列表已满");
						return;
					}
					...
					角色数据.删除日期.V = (角色数据.冻结日期.V = default(DateTime));
					角色数据.所属账号.V.删除列表.Remove(角色数据);
					角色数据.所属账号.V.角色列表.Add(角色数据);
					主程.添加命令日志("<= @" + base.GetType().Name + " 命令已经执行, 角色恢复成功");
				}
				else
				{
					主程.添加命令日志("<= @" + base.GetType().Name + " 命令执行失败, 角色未被删除");
				}
			}
			else
			{
