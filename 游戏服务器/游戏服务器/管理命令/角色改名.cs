		...
		public override void 执行命令()
		{
			if (游戏数据网关.角色数据表.检索表.TryGetValue(this.角色名字, out var value) && value is 角色数据 角色数据)
			{
				if (角色数据.网络连接 == null && 角色数据.所属账号.V.网络连接 == null)
				{
					...
					{
						主程.添加命令日志("<= @" + base.GetType().Name + " 命令执行失败, 角色名字太长");
						return;
					}
					if (游戏数据网关.角色数据表.检索表.ContainsKey(this.新角色名))
					{
						主程.添加命令日志("<= @" + base.GetType().Name + " 命令执行失败, 名字已被注册");
						return;
					}
					游戏数据网关.角色数据表.检索表.Remove(角色数据.角色名字.V);
					角色数据.角色名字.V = this.新角色名;
					游戏数据网关.角色数据表.检索表.Add(角色数据.角色名字.V, 角色数据);
					主程.添加命令日志($"<= @{base.GetType().Name} 命令已经执行, 角色当前名字: {角色数据}");
				}
				else
				{
					主程.添加命令日志("<= @" + base.GetType().Name + " 命令执行失败, 账号必须下线");
				}
			}
			else
			{
